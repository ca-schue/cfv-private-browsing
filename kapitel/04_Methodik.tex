...
Chrome basiert auf dem Open-Source-Projekt \textit{Chromium}, einem freien und quelloffenen Webbrowser, der von \textit{The Chromium Projects} entwickelt wird.
...
Um vor dem Webangreifer zu schützen, bietet Brave mit dem \textit{Private-Window-with-Tor-Connectivity}-Modus zusätzlich die Möglichkeit, eine Verbindung zum Tor-Netzwerk herzustellen. \cite{Brave.} Im Rahmen dieser Seminararbeit wird für Brave ausschließlich der Private-Window-Modus ohne Verbindung zum Tor-Netzwerk untersucht.
...
\item \textbf{Chrome}: \texttt{C:$\backslash$Program Files$\backslash$Google$\backslash$Chrome$\backslash$Application$\backslash$chrome.exe}
\item \textbf{Brave}: \texttt{C:$\backslash$Program Files$\backslash$BraveSoftware$\backslash$Brave-Browser$\backslash$Application$\backslash$brave .exe}
\end{enumerate}
...
Um das Verhalten von privaten Browsing-Modi möglichst vollständig zu untersuchen, schlagen Fayyad-Kazan et al. \cite{Fayyad.2021} vor, alle Aktivitäten des Browsers während des Browsing-Szenarios aufzuzeichnen.
...
Durch Nutzung des VirtualBox Befehlszeilen-Tool \textit{vboxmanage} wird mit dem Befehl \texttt{vboxmanage clonehd <VDI\_File>.vdi <IMG\_File>.img --format raw} die VDI-Datei in eine IMG-Datei umgewandelt.
...
Das gefilterte Logfile wird als CSV exportiert, um es dann in Excel zu öffnen und irrelevante Spalten sowie Duplikate zu löschen.
...
\begin{figure}[h!]
	...
	\caption{Vorgehen zur Dateiextraktion und -analyse}
...
In der betrachteten Literatur wird die sogenannte \textit{pagefile.sys} als Teil der Uncommon Locations unter Windows betrachtet. Diese Datei wird verwendet, um Daten aus dem Hauptspeicher auf die Festplatte auszulagern, wenn der verfügbare physische Speicher begrenzt ist. \cite{MicrosoftLearn.21.03.2023}
...
} oder andere Festplattenbereiche, wie beispielsweise unallokierte Speicherbereiche oder den Arbeitsspeicher.
